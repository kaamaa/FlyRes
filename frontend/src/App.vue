...
function onSaved() { view.value = 'meine'; showToast('Reservierung gespeichert'); bump.value++ }
...
<template>
  <div class="approot">
    ...
    <template v-else>
      <MyFlightsView v-if="view === 'meine'" :key="'m' + bump" @open="openDetail" @reserve="view = 'neu'" />
      <AllView v-else-if="view === 'alle'" :key="'a' + bump" :md="md" @open="openDetail" />
      <ReserveView v-else-if="view === 'neu'" :key="'r' + bump" :md="md" @saved="onSaved" />

      <TabBar :view="view" @change="view = $event" />
